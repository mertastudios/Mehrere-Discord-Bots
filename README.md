...
| 🛡️ **Security Bot** | **KI-Sicherheitsbot mit Google Gemini** (Standardmodell `gemini-flash-lite-latest` – ein von Google gepflegter Alias, der immer auf die aktuell günstigste Flash-Lite-Generation zeigt, plus automatischer Modell-Fallback bei 404 "nicht mehr verfügbar") – sammelt Textnachrichten echter Nutzer (Admins immun, aber als Kontext ohne ID sichtbar), analysiert sie **adaptiv und deutlich häufiger** (Risikosignale/Dogpiling/Repeated Mentions sofort, kurze Verläufe nach kurzer Wartezeit, 2-Stunden-Flush nur als Sicherheitsnetz) und schickt den sauber formatierten Verlauf (IDs ab 1, Mentions/Replies/Nicknames/Namen aufgelöst, nach Kanälen sortiert) inkl. System-Prompt + Server-Regeln an Gemini. Gemini entscheidet über **Warnung/Timeout (1m–1w)** pro Nutzer mit persönlicher Nachricht, schützt harmlose Jokes/Sarkasmus, prüft aber explizit Mobbing/Dogpiling/Nachtreten. Der Bot antwortet auf den schwerwiegendsten Verstoß – **Maßnahme und Hauptgrund immer fett formatiert**. Googles Safety-Filter sind komplett deaktiviert (`OFF`/`BLOCK_NONE` inkl. Civic-Integrity) und leere Antworten (`finish_reason: SAFETY`) werden über eine Varianten-Leiter automatisch gerettet, damit gerade schwere Verstöße moderierbar bleiben. Ohne Verstoß bleibt er **komplett still** (kein Small-Talk im Chat). Strafenregister (20 Tage) sichtbar für die KI. Bei API-Fehlern geht **nichts verloren** (Retry-Queue mit Backoff + lokaler Gemini-Rate-Limiter). Commands (alle Admin-only): `/set_gemini_api_key`, `/set_prompt` (Formular), `/set_log_channel`, `/set_language` (10 Sprachen), `/security_check_now` (sofortige Analyse ohne zu warten), `/security_action` (**verdeckte KI-Moderation**: letzte ~200 Nachrichten eines Mitglieds scannen, gewünschte Nachrichten seitenweise auswählen – die KI sucht den schwersten Verstoß, entscheidet und begründet, als hätte sie es selbst bemerkt), `/security_ai_order` (freier KI-Auftrag mit Begründung zum gesamten Chatverlauf), `/security_status`, `/help`. Log-Kanal meldet Moderationen & API-Fehler. Turso-persistiert, kein Kick/Ban. |
...
│   │   ├── index.js
│   │   └── src/              # gesamte Bot-Logik
│   ├── security-bot/         # 🛡️ Sicherheits-Bot (KI-Moderation mit Google Gemini, Turso-persistiert)
│   │   ├── index.js
│   │   └── src/              # Sammler, Gemini-Client, Prompt-Bau, Retry-Pipeline, 10 Sprachen
