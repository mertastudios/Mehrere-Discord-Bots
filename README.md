...
| 🛡️ **Security Bot** | **KI-Sicherheitsbot mit Google Gemini** (Standardmodell `gemini-flash-lite-latest` – ein von Google gepflegter Alias, der immer auf die aktuell günstigste Flash-Lite-Generation zeigt, plus automatischer Modell-Fallback bei 404 "nicht mehr verfügbar") – sammelt Textnachrichten echter Nutzer (Admins immun, aber als Kontext ohne ID sichtbar), bis das Token-Budget voll ist bzw. **jede Nacht um 0 Uhr**, und schickt den sauber formatierten Verlauf (IDs ab 1, Mentions aufgelöst, nach Kanälen sortiert) inkl. System-Prompt + Server-Regeln an Gemini. Gemini entscheidet über **Warnung/Timeout (1m–1w)** pro Nutzer mit persönlicher Nachricht, der Bot antwortet auf den schwerwiegendsten Verstoß. Strafenregister (20 Tage) sichtbar für die KI. Bei API-Fehlern geht **nichts verloren** (Retry-Queue mit Backoff). Commands (alle Admin-only): `/set_gemini_api_key`, `/set_prompt` (Formular), `/set_log_channel`, `/set_language` (10 Sprachen), `/security_check_now` (sofortige Analyse ohne zu warten), `/help`. Log-Kanal meldet Moderationen & API-Fehler. Turso-persistiert, kein Kick/Ban. |
